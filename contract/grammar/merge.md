...
1. If `schema_version` is outside the accepted range, exit with code 3 and name both the report's version and the accepted range (Requirement 30.15).
2. If `analyzer.conformance` is missing, or its `result` is not `pass`, exit with code 3 and name the analyzer (Requirement 30.14).

Admission runs over every report before any other step. A failure here produces no merged report. Exit code: 3.

### Step 2: Union

Carry every finding, every stale-suppression record and every declared gap from every input into the merged report, without deduplication (Requirement 30.17). Two analyzers that claim one language (Requirement 30.4) and report the same finding produce two records, and step 6 orders them by the analyzer name. Carry every edge evaluation into a working set for steps 3 to 5. Exit code: none.

### Step 3: Index

Build an index from the working set: `index[edge][side]` is the list of every evaluation across every report that names that edge identifier and that side (Requirements 31.6, 31.11). A side holds more than one evaluation when more than one analyzer claims its language. A side holds none when no analyzer that ran claims its language; a side with no evaluation is not `absent`, because no analyzer looked. Exit code: none.
...
1. `other` is empty. Exit with code 3 and name the unresolved edge, the pending side and its symbol (Requirement 31.8). No report in the merge evaluated the paired side, so no answer exists.
2. Any evaluation in `other` is `live`. Drop the finding. It appears nowhere in the merged report (Requirement 31.6).
3. Any evaluation in `other` is `dead`. Promote the finding into `findings` (Requirement 31.6). The paired symbols join one component under the rule below (Requirement 31.7).
4. Every evaluation in `other` is `absent`. Drop the finding. The edge names a symbol no analyzer enumerates, and step 5 reports the edge (Requirement 31.9). The finding is not promoted, because the stale edge is the defect to fix first: a misspelled paired reference would otherwise turn a live pairing into a reported deletion.
...
Sort `findings` (carried, promoted and emitted together), `stale_suppressions` and `declared_gaps` by the canonical key below (Requirement 2.13). The merged report's `edge_evaluations` array carries every evaluation whose state is `live` or `absent`, ordered by edge identifier, then side, then the carrying analyzer's name, each compared bytewise; it carries no evaluation whose state is `dead`, because step 4 resolved each one into a promotion, a drop or a stale edge. No evaluation carries a finding, so the merged report holds no pending finding (Requirement 31.10). Recompute `totals` over the merged arrays. The remaining envelope members, including the version and digest of each analyzer that ran (Requirement 30.20), are declared by `report.schema.json` and are derived from the input reports alone. Exit code: none.

### Step 7: Verdict

Apply the exit-code table to the merged report (Requirement 30.16). Return 1 when at least one finding carries the `deny` severity or `stale_suppressions` is non-empty (Requirements 25.2, 25.9). Return 0 otherwise (Requirement 25.1). A `warn` finding never fails the run (Requirement 25.6). Where the caller's configuration turns the exit code off, the verdict is 0 and the report is unchanged (Requirement 25.5).
...
1. Admission.   for each r in reports:
                  r.schema_version not in accepted_schema_range -> exit 3       # 30.15
                  r.analyzer.conformance.result != "pass"       -> exit 3       # 30.14
2. Union.       carry every finding, stale suppression and declared gap,
                without deduplication                                           # 30.17
3. Index.       index[edge][side] = [evaluation, ...] over every report          # 31.6, 31.11
4. Resolve.     for each dead evaluation e, in canonical order of e.finding:
                  other = index[e.edge][side != e.side]
                  other empty            -> exit 3, name the edge                # 31.8
                  any(other) live        -> drop e.finding                       # 31.6
                  any(other) dead        -> promote e.finding                    # 31.6
                  all(other) absent      -> drop e.finding; step 5 reports       # 31.9
                union the components of promoted paired findings,
                in canonical order                                               # 31.7
5. Stale edges. for each edge with a side whose every evaluation is absent:
                  emit one DS1705                                                # 31.9
6. Order.       sort findings, stale_suppressions and declared_gaps by the
                canonical key; carry live and absent evaluations only            # 2.13, 31.10
7. Verdict.     any deny finding or any stale suppression -> 1, else 0           # 25, 30.16
```
...
`analyzer.name` is the final tiebreak because Requirement 30.4 permits two analyzers claiming one language and Requirement 30.17 obliges the merge to keep both their findings, so an identical finding from two analyzers appears twice, adjacent and in a fixed order. That consequence is checkable: configure two Go analyzers and every shared finding doubles.

Language is deliberately absent from the key. Two languages never share a path, so `path` already separates a Go finding from a TypeScript one, and a mixed repository reads better with a Go handler next to the TypeScript client generated from it than as two language blocks. Nothing is lost by the omission: each finding names its language in its own `language` field (Requirement 31.3), and symbol references are namespaced by language (Requirement 31.2), so two same-named symbols in two languages already differ on `symbol.ref`.
...
## Step, requirement and exit code

| Step | Answers | Exit code it can produce |
| --- | --- | --- |
| 1. Admission | 30.14, 30.15 | 3 |
| 2. Union | 30.17 | none |
| 3. Index | 31.6, 31.11 | none |
| 4. Resolve | 31.6, 31.7, 31.8, 31.9 | 3 |
| 5. Stale edges | 31.9 | none |
| 6. Order | 2.13, 31.10 | none |
| 7. Verdict | 25.1, 25.2, 25.5, 25.6, 25.9, 30.16 | 0 or 1 |
