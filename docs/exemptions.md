...
Mechanism in Go: A type T flows into the class when a value of T, or a pointer to one, reaches one of the destinations below. Retain on T the members that destination reads, and retain the same members on every defined type reached from the fields of a retained type through a pointer, an array, a slice, a map key or value, or an embedded field, applied until no further type joins. The reach stops at an interface-typed field, whose dynamic type the analysis does not see. Fields alone are retained for a destination that reads fields and resolves no method by name: an argument of a function or method of `encoding/json`, `encoding/xml` or `encoding/gob`, a `database/sql` `Scan` target, an argument of `reflect.DeepEqual`, and an argument of any function or method of `reflect` other than the method-reaching calls below. Fields and the `LogValue() slog.Value` method are retained for a `log/slog` destination: an argument of a `log/slog` logging function, of `slog.With`, of a `*slog.Logger` method, `With` among them, of `(*slog.Record).Add` or of an attribute constructor. A handler resolves `LogValue` by the value's own shape and not through a conversion, so no conversion exists for another class to read and this class is the only one that retains it. Fields and exported methods are retained for a destination that resolves a method by name: an argument of a function or method of `text/template` or `html/template`, and a value reaching a method-reaching `reflect` call, being `Method`, `MethodByName` or `NumMethod` on a `reflect.Value` or a `reflect.Type`, or a `reflect.Value` the program calls a method through. Fields and exported methods are retained for a destination outside the analyzed program: a value of a struct type, or a pointer to one, passed as an interface-typed argument to a function or method of a package that is neither the target nor a loaded consumer, recorded at the call with the detail naming the callee. The analysis holds no syntax for that function, so it is read as reading every field the class retains and as resolving any exported method by name; and any function of the analyzed program whose interface-typed parameter it passes to such a call, or to any other destination of this class, is itself a destination, applied until no further function joins. A conversion to `sort.Interface` is an ordinary interface conversion rather than a destination of this class, so the methods that interface requires are retained by `interface-satisfaction`.
...
A class whose instances flow into a serializer or a schema validator has its data members retained, because the serializer reads them by name at runtime; those destinations retain no method, because a serializer calls none. A destination outside the analyzed program retains more: a value passed to a parameter typed `unknown` or `any` of a function of a module that is neither the project nor a declared consumer has left the analysis, so its properties are retained together with the two conversion methods a serializer or a formatter resolves by name.
