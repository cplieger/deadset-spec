...
```text
go://github.com/cplieger/toolbelt/v3#Catalog.ResolveAlias
ts://@cplieger/vibekit-web/src/features/tabs/index.ts#TabStrip.cachedLayout
```
...
1. **The language prefix is the namespace.** `go` covers Go; `ts` covers TypeScript and JavaScript, because one analyzer covers both and `kinds.json` names its language `ts`. The prefix is lowercase and is followed by `://`. A third language joins by adding a prefix and a section to this page, and nothing else changes: an edge pairs two references and names no language in its own shape (Requirement 31.1).
2. **The first `#` splits the scope from the fragment.** The scope never contains `#`; the fragment may, in a TypeScript private name.
3. **An empty fragment names the scope itself**: the Go package, or the TypeScript module. The `#` is still written, so that a scope is never mistaken for a symbol. API Extractor fixes the same rule for the reference `controls#` and says why: without the `#`, `controls` reads as a symbol of that name ([API Extractor, declaration references](https://api-extractor.com/pages/tsdoc/declaration_references/)).
...
The reference is built from names and containers only: the scope, the declared name, and the chain of containers between them. No line, no column, no byte offset and no index enters it. An edit above a declaration, a reformat, or a reordering of the fields of a struct or the members of a class leaves every reference in the file unchanged, so a suppression, a root or an edge written against it still matches (design Property 32). Three edits change a reference, and each is a change to the thing named: renaming the declaration, moving it to another package or module, and changing its container, such as moving a method to another type.
...
Two position-free alternatives were read before this grammar was fixed, and the reasons they were passed over decide two rules below. [`golang.org/x/tools/go/types/objectpath`](https://pkg.go.dev/golang.org/x/tools/go/types/objectpath) encodes an object as a chain of operators from the package scope, and its [source](https://github.com/golang/tools/blob/master/go/types/objectpath/objectpath.go) encodes a struct field as `F` plus an index and a method as `M` plus an index, so inserting a field above another changes the second field's path; its documentation also states that `For` returns no path for an unexported package-level function or variable, which `DS1002` reports, and that one object may have several paths. So this grammar names members by name, never by index, and the Go analyzer carries the object path as a second key, `symbol.objectpath`, only where one exists. [`go/types.ObjectString`](https://pkg.go.dev/go/types#ObjectString) renders `func (*bytes.Buffer).ReadFrom(r io.Reader) (n int64, err error)`: it carries the signature, which changes when a parameter is added, and with the default qualifier it joins the import path and the name with a dot, which is ambiguous on a path such as `gopkg.in/yaml.v3`. So this grammar carries no signature and separates the scope from the fragment with `#`, the character godoc has used for the same job since its anchors took the shape [`bytes#Buffer.ReadFrom`](https://pkg.go.dev/bytes#Buffer.ReadFrom).
...
The scope is the import path of the package that declares the symbol: the module path from `go.mod`, then `/` and the package's directory below the module root, which is what `go list` reports as `ImportPath` and what `types.Package.Path()` returns. The package at the module root has the module path alone as its scope. A `main` package is scoped like any other. The corpus carries `go://github.com/cplieger/toolbelt/v3/cmd/toolcatalog#run`.

Two test packages exist per directory and they scope differently. A test file whose package clause names the production package (`package toolbelt`, in `catalog_test.go`) is a variant of the production package: the analyzer keys its declarations by position so each appears once (Requirement 4.6), and its symbols share the production scope, `go://github.com/cplieger/toolbelt/v3#TestResolve`. A test file whose package clause carries the `_test` suffix (`package httpapi_test`) is a separate package whose path is the directory path with that suffix, `go://github.com/cplieger/toolbelt/v3/httpapi_test#TestSearch`. Neither carries the bracketed test-binary suffix that `go list -test` appends to the import path and `go/packages` keeps in a package's `ID`: the reference uses `PkgPath`, which is `github.com/cplieger/toolbelt/v3` for the variant and `github.com/cplieger/toolbelt/v3/httpapi_test` for the external package. The synthetic `main` package the toolchain builds for a test binary declares nothing of the target and is never a scope.
...
```text
go://github.com/cplieger/toolbelt/v3#Resolve
go://github.com/cplieger/toolbelt/v3#normalize
go://github.com/cplieger/langtag#Exact
```
...
**A method** is `Type.Method`, with no receiver star and no type arguments: `Catalog.ResolveAlias` for `func (c *Catalog) ResolveAlias`, and `List.Push` for `func (l *List[T]) Push`. This is the anchor godoc renders and the form `cmd/deadcode` prints (`Parsed.WriteNode` in its [documented output](https://pkg.go.dev/golang.org/x/tools/cmd/deadcode)), and the star carries nothing because the language forbids a value-receiver and a pointer-receiver method of one name on one base type ([Go specification, method declarations](https://go.dev/ref/spec#Method_declarations)). The display name `(*Catalog).ResolveAlias` keeps the star and is `symbol.name`, which [`text-line.md`](text-line.md) renders and this page never carries. A method declared on an alias of a defined type belongs to the defined type, so its container is that type's name. An interface method takes the same form, `Fetcher.Fetch`.

**A struct field** is `Type.Field`. An embedded field is spelled by its field name, which the language defines as the unqualified type name ([Go specification, struct types](https://go.dev/ref/spec#Struct_types)): `Buffered.Buffer` for `*bytes.Buffer` embedded in `Buffered`. A field of an anonymous struct type is reached through the field that carries the type, whatever pointer, slice, array, channel or map-value wrappers sit between: `Manifest.Tools.Version` for `Tools []struct{ Name, Version string }`. A field always carries its struct. `punused` printed `field Detail is unused` with no container ([its README](https://github.com/bep/punused)), so two structs with a `Detail` field printed the same line, and the Language Server Protocol says of its `containerName` that it is for display and cannot re-infer a hierarchy ([LSP 3.17, `SymbolInformation`](https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#textDocument_documentSymbol)); the reference carries the whole chain instead, so it identifies rather than labels.
...
A Go enumerated member is a package-level constant and takes the package-level form. Type arguments never appear on a container, in either language; TSDoc states the same rule for TypeScript, that `<T>` is never part of a declaration reference, and the reason holds for Go as well: the type parameter list is part of the declaration, not of its identity.

### The package, a file and a module directive

The package itself is the empty fragment, `go://github.com/cplieger/toolbelt/v3#`. It is the `symbol.parent` of every package-level declaration.

A source file is its base name followed by the `:file` selector, scoped by the import path of the directory that holds it, whatever package clause the file carries: `go://github.com/cplieger/toolbelt/v3/internal/legacy#render_windows.go:file`. The selector exists because a file name is not an identifier chain and must not be read as one: `render_windows.go` without the selector parses as the member `go` of `render_windows`. Folding the file name into the scope instead was refused because an import path element may legally end in `.go`, so `go://…/legacy/render_windows.go#` would be the spelling of a package that does not exist. The selector shape, an operand followed by a colon and a lowercase word, is API Extractor's `Meaning` suffix (`:class`, `:function(1)`) in its [declaration-reference grammar](https://github.com/microsoft/tsdoc/blob/main/tsdoc/src/beta/DeclarationReference.grammarkdown), reused here for every subject that is not an identifier chain.

A `require` directive is the required module path followed by `:require`, scoped by the module whose `go.mod` carries it, which is the module path itself: `go://github.com/cplieger/toolbelt/v3#github.com/example/left:require`. The required version is the directive's value, not its identity, so it never appears: a bump leaves the reference unchanged. A `replace` directive is the replaced module path, then `@` and the version when the directive names one, then `:replace`: `go://github.com/cplieger/toolbelt/v3#github.com/old/mod@v1.2.3:replace`, or `#github.com/old/mod:replace` for a directive with no version. The version stays on a `replace` because two `replace` lines for one module at two versions are two directives. A nested module inside the target is its own scope. No other directive has a form, because no other directive is a finding subject (`DS1605`'s precondition in `kinds.json`).
...
- **A declaration inside a function body**: a local type, constant, variable or closure. `objectpath` returns no path for local names, and `cmd/deadcode` names a closure by an ordinal, `goListDriver$1`, which changes when a closure is added above it. The enclosing function's reference stands for it.
...
The scope is the module: the name of the package the file belongs to, then `/`, then the file's path inside that package, with `/` as the separator and the file's own extension. The package name is the `name` field of the nearest `package.json` at or above the file's directory, up to the target root, and the path is relative to that manifest's directory. TSDoc's declaration references take the same shape, `@microsoft/my-control-library/lib/Button#Button` ([TSDoc, `@link`](https://tsdoc.org/pages/tags/link/)), with one difference stated below.

```text
ts://@cplieger/vibekit-web/src/wire.ts#
ts://@cplieger/vibekit-web/src/features/tabs/index.ts#TabStrip
```
...
The path names a source file, never an import specifier, so the extension is always present and is one of `.ts`, `.tsx`, `.mts`, `.cts`, `.d.ts`, `.d.mts`, `.d.cts`, `.js`, `.jsx`, `.mjs` or `.cjs`. This is the difference from TSDoc: TSDoc resolves a reference against a package's entry point and forbids a file-relative shorthand, because a documentation tool needs to link without a compiler; this grammar names the declaring file, because a module-local declaration that is not exported has no entry-point path at all, and `DS1002` reports exactly those. The `package.json` itself is a scope for one form only, the dependency form below.

A JavaScript file is scoped and spelled exactly as a TypeScript file, under the `ts` prefix: `ts://@cplieger/vibekit-web/scripts/build.mjs#bundle`.
...
- **An identifier name** is written bare: `TabStrip`, `default`, `static`, `größe`. The test is the ECMAScript `IdentifierName` production ([ECMA-262](https://tc39.es/ecma262/#prod-IdentifierName)), which admits reserved words, so `default` and `static` are bare. TSDoc applies the same test and writes `ClassJ2.static` and `ClassJ2.spaß` bare.
...
An instance member is bare and **a static member always carries `:static`**, because a class may declare a static and an instance member of one name and the spelling must not depend on whether it does. TSDoc's `(render:instance)` and `(render:static)` selectors mark both sides and are optional when no collision exists; here one side is marked and always marked, so the reference is canonical. A getter and a setter of one name are one symbol and one reference. A private modifier (`private cachedLayout`) changes nothing in the spelling; only the `#` form is spelled differently, because only it is a different name.
...
The module itself is the empty fragment, `ts://@cplieger/vibekit-web/src/wire.ts#`. It is the `symbol.parent` of every module-level declaration and the reference of a file-level finding (`DS1501`, `DS1502`).
...
```text
ts://@cplieger/vibekit-web/package.json#lodash:dependency
ts://@cplieger/vibekit-web/package.json#@types/node:dev-dependency
```
...
- **An overload signature.** A function with three overload signatures is one symbol with one reference. TSDoc offers `(functionC1:1)` index selectors and marks them not recommended, and API Extractor does not support them; an index shifts when a signature is inserted above it, which is the instability this grammar exists to avoid.
...
The same fourteen expressions with every block substituted, one per line under its language and form, for copying into a test. Every line compiles under Go's `regexp` and under an ECMAScript `RegExp` with no flags, and the corpus was run through both before this page was fixed.
...
[`symbol-ref-corpus.json`](symbol-ref-corpus.json) is an array of cases. Each case carries the `input` string, its `language`, the `form` it takes or nearly takes, whether it is `accepted`, a `reason`, and for an accepted case an `example` of one or two source lines the reference denotes. Every form above has at least one accepted case, and every rule has a refused near-miss: the display name with its receiver star, a parameter in parentheses, a field written with a slash, a trailing dot, TSDoc's parenthesized selector, API Extractor's `#` and `!`, a double-quoted string, a Go bracket on a TypeScript type parameter, a percent-encoded identifier, a stray CR or LF, and a spelling in which the selector precedes its operand.

## How the shape compares with other tools

Every rule above was taken from a tool that already names symbols, or was fixed against one. Whoever knows one of these knows most of this grammar.

| Tool | Names a symbol as | Taken, or fixed against |
| --- | --- | --- |
| godoc anchors (`pkg.go.dev`) | `#Type.Method`, `#Func` | The `#` separator and the star-free method form. |
| Go doc links ([go.dev/doc/comment](https://go.dev/doc/comment#doclinks)) | `[pkg.Name1.Name2]`, `[example.com/sys.File]` | The full import path as the scope. |
| [`go/types.ObjectString`](https://pkg.go.dev/go/types#ObjectString) | `func (*bytes.Buffer).ReadFrom(r io.Reader) …` | Fixed against: no signature, and `#` rather than a dot after a path that may itself contain dots. |
| [`objectpath`](https://pkg.go.dev/golang.org/x/tools/go/types/objectpath) | `T.UM0.RA1.F0` | Fixed against: names rather than indices, a form for unexported non-types; kept as `symbol.objectpath` where one exists. |
| [`cmd/deadcode`](https://pkg.go.dev/golang.org/x/tools/cmd/deadcode) | `Path` plus `Parsed.WriteNode`; closures as `f$1` | The `Type.Method` form; fixed against the ordinal for closures. |
| [`punused`](https://github.com/bep/punused) | `field Detail`, `method (MyType).UnusedMethod` | Fixed against: a member always carries its container, and the star is display only. |
| [TSDoc declaration references](https://github.com/microsoft/tsdoc/blob/main/spec/code-snippets/DeclarationReferences.ts) | `pkg/path#Ns.Class.member`, `"quoted"`, `(x:static)`, `[symbol]`, `EnumH1.memberH2` | The scope shape, the quoting test, the computed-key brackets, the enum and namespace chains, generics never appearing, and the ruling that overload indexes are not stable. Fixed against: double quotes, and the optional selector. |
| [API Extractor canonical form](https://github.com/microsoft/tsdoc/blob/main/tsdoc/src/beta/DeclarationReference.grammarkdown) | `pkg!Ns.Class#member:member(1)`, `~local` | The `:selector` suffix. Fixed against: `!` and the `#` member navigation, and marking locals rather than aliases. |
| [knip `json` reporter](https://knip.dev/features/reporters) | `{ file, name, namespace? }` | Fixed against: a bare `name` per file, which is why `ignoreMembers` by name masked matches across files. |
| [LSP `SymbolInformation`](https://microsoft.github.io/language-server-protocol/specifications/lsp/3.17/specification/#textDocument_documentSymbol) | `name` plus a display-only `containerName` | Fixed against: the whole container chain, because the protocol states its own field cannot re-infer a hierarchy. |
