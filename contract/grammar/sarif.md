...
| `properties` | Every field of the finding this page has not mapped above, under its schema name and with its value unchanged. At schema version 1.0.0 that is `language`, `symbol`, `reachability_class`, `confidence`, `liveness_relation`, `test_only`, `generated`, `component`, `retained_by`, `configurations`, `consumers_loaded`, `fixability` and `details`. GitHub does not read the bag; it serves other SARIF consumers, a codemod and a reader of the file, and it changes nothing about the alert GitHub shows. |
